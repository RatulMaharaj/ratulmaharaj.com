...
Functions are useful for creating reusable code. They can be used to perform a specific task. They can also be used to perform a specific task multiple times. They can also be used to perform a specific task multiple times with different parameters.
...
* `type()` which returns the type of an object etc.
...
They can take arguments and return values. The `return` keyword is used to return a value from a function. If no value is returned, the function returns `None`. The `return` keyword can also be used to exit a function early.
